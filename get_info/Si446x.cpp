...
#define spiSelect()				(CSN_PORT &= ~_BV(CSN_BIT)) //find port addresses specific to ARM M3  
...
void Si446x_init()
{
	...
	SPI.begin();
#else
	// The BV macro is making the mask you are ORing with the direction register of the CSN
	CSN_DDR |= _BV(CSN_BIT); //BV  performs a bit shift #define _BV(bit) (1 << (bit))
	SDN_DDR |= _BV(SDN_BIT);
